 ...
 * arg2 is used to tell the function the tag to modify, arg3 is used only for modal 2 and these settings (arg3  0--->1)
...
function modal2_on() {
    ...
    function processData(event) {
        ...
        console.log("file from drop",file)//ajout pour debug
        formData.append("image",file)//add file to formdata in order to be sent to the serveur
        console.log("formdata from drop = ",formData)//ajout pour debug
    ...
    */
    function previewFile(arg = 0) {
        ...
        file_reader.addEventListener("load", (event) => imageAndForm_manage(event, file))//when file is loading--->executes imageAndForm_manage
      /*   console.log (file)
        ...
        */
        function imageAndForm_manage(event, file) {
            ...
            img.src = event.target.result//get the image which is loading
            /* console.log("RRRR",img.src) */
           /*  sessionStorage.setItem("image3",event.target.result) */ //supprimmé
...
displayGallerySearch(); //call function to display search buttons,this function manage clicks on buttons too
